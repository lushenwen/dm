...
router.get('/add',(req,res)=>{
    ...
    var sql=`SELECT iid FROM lsm_shoppingcart_item WHERE user_id=? AND product_id=?`;
    pool.query(sql,[req.session.loginUid,$lid],(err,result)=>{
        if(err) throw err;
        var sql2;
        if(result.length>0){
            sql2=`UPDATE lsm_shoppingcart_item SET count=count+1 WHERE user_id=${req.session.loginUid} AND product_id=${$lid}`;

        }else{
            sql2=`INSERT INTO lsm_shoppingcart_item VALUES(NULL, ${req.session.loginUid}, ${$lid}, ${$buyCount}, false)`;
        }
...
router.get('/list',(req,res)=>{
    ...
    var sql='SELECT iid,lid,title,spec,price,count FROM lsm_laptop l, lsm_shoppingcart_item s WHERE l.lid=s.product_id AND user_id=?';
    pool.query(sql,[req.session.loginUid],(err,result)=>{
        ...
        for(var i=0;i<output.data.length;i++){
            ...
            (function(lid,i){
              pool.query('SELECT sm FROM lsm_laptop_pic WHERE laptop_id=? LIMIT 1',[lid],(err,result)=>{
...
router.get('/del',(req,res)=>{
    ...
    pool.query('DELETE FROM lsm_shoppingcart_item WHERE iid=?',[obj.iid],(err,result)=>{
        if(err) throw err;
...
router.get('/updatecount',(req,res)=>{
    ...
    pool.query('UPDATE lsm_shoppingcart_item SET count=? WHERE iid=?',[obj.count,obj.iid],(err,result)=>{
        if(err) throw err;
        if(result.affectedRows>0){
...
router.get('/updatechecked',(req,res)=>{
    ...
    pool.query('UPDATE lsm_shoppingcart_item SET is_checked=? WHERE iid=?',[obj.checked,obj.iid],(err,result)=>{
        if(err) throw err;
        if(result.affectedRows>0){
            res.send({code:200,msg:'update succ'});
        }else{
...
router.get('/listchecked',(req,res)=>{
    ...
    var sql = "SELECT iid,lid,title,spec,price,count FROM lsm_laptop l, lsm_shoppingcart_item s WHERE l.lid=s.product_id AND user_id=? AND is_checked=1";
    pool.query(sql,[req.session.loginUid],(err,result)=>{
        if(err) throw err;
        output.code=200;
